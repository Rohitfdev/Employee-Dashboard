...
function DailyTimesheetTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const timesheetResponse = await fetch("https://66ba1676fa763ff550fac017.mockapi.io/daily-timesheet");
  ...
  const enrichedUsers = users.map((user) => {
    const userTimesheet = timesheetData.find(
      (timesheet) => timesheet.employeeId === user.id.toString() // Ensure the types match
    );
    ...
    return {
      ...user,
      workHours: userTimesheet ? userTimesheet.workHours : "No timesheet records available",
      timesheetDate: userTimesheet ? formatDate(userTimesheet.date) : "N/A",
    };
  });
  ...
  const handleDownloadExcel = () => {
    ...
    saveAs(fileData, "timesheet_records.xlsx");
  };
