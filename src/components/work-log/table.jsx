...
function WorkLogTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const workLogResponse = await fetch("https://66ba1676fa763ff550fac017.mockapi.io/work-log");
  ...
  const enrichedUsers = users.map((user) => {
    const userWorkLog = workLogData.find(
      (workLog) => workLog.employeeId === user.id.toString() // Ensure the types match
    );
    ...
    return {
      ...user,
      logHours: userWorkLog ? userWorkLog.logHours : "No work log records available",
      logDate: userWorkLog ? formatDate(userWorkLog.logDate) : "N/A",
    };
  });
  ...
  const handleDownloadExcel = () => {
    ...
    saveAs(fileData, "work_log_records.xlsx");
  };
