...
function ReportsTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const reportsResponse = await fetch("https://66ba4856fa763ff550fb68b8.mockapi.io/reports");
  ...
  const enrichedUsers = users.map((user) => {
    const userReport = reportsData.find(
      (report) => report.employeeId === user.id.toString() // Ensure the types match
    );
    ...
    return {
      ...user,
      reportType: userReport ? userReport.reportType : "No report records available",
      reportDate: userReport ? formatDate(userReport.date) : "N/A",
      description: userReport ? userReport.description : "N/A",
    };
  });
  ...
  const handleDownloadExcel = () => {
    ...
    saveAs(fileData, "report_records.xlsx");
  };
